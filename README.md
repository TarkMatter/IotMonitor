...
工場・倉庫・クリーンルーム・オフィスにおける **温度・湿度・CO₂** のリアルタイム収集・可視化・アラート通知システム。
...
| CO₂ センサ | MH-Z19B (UART) | CO₂ 濃度測定 |

材料費目安: **約 ¥11,000〜**

### センサなし（デモモード）

`simulator.py` が MQTT に模擬データを送信するため、PC 単体で全機能を動作確認できる。
...
| `tests/test_thresholds.py` | 閾値チェック | 16 |
| `tests/test_api.py` | FastAPI エンドポイント | 14 |
| `tests/test_simulator.py` | シミュレーター | 10 |
| `tests/test_alerter_main.py` | アラータークールダウン | 11 |
| `tests/test_reporter.py` | Excel レポート生成 | 15 |
